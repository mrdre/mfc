...
var ajaxChat = {
	...
	updateChat: function(paramString) {
		...
		if (this.contains(this.userNamesList,'admin') || this.vojarUser == true  ){
			ajaxChat.blackshirma();
			
		} else {
				ajaxChat.greenshirma();
				ajaxChat.show = true;
		}
		ajaxChat.privatemessageUpdate();
	...
	handleXML: function(xmlDoc) {
		this.zapmsg = xmlDoc.getElementsByTagName('message');
	...
	getUserNodeStringItems: function(encodedUserName, userID, isInline) {
		var menu;
		if(encodedUserName != this.encodedUserName && this.userRole != 0) {
			menu 	=
			// '<li><a href="javascript:ajaxChat.insertMessageWrapper(\'/msg '
					...
					+ '</a></li>';
				if (encodedUserName == 'admin'){//(this.userRole !=3){
			...
			if(this.userRole == 2 || this.userRole == 3) {
				menu	+= '<li><a href="javascript:ajaxChat.insertMessageWrapper(\'/msg '
						...
						+ this.lang['userMenuSendPrivateMessage']
						+ '</a></li>'
						+'<li><a href="javascript:ajaxChat.insertMessageWrapper(\'/kick '
						+ encodedUserName
						+ ' \');">'
			...
			if(this.userRole == 1 || this.userRole == 2 || this.userRole == 3) {
				// if(encodedUserName != this.encodedUserName && this.userRole == 2 || this.userRole == 3) {
	...
	if (this.userRole != 0){
		...
		alert('Please register to send messages :)');
		}
	},
	...
	replaceCommands: function(text) {
		try {
			...
			switch(textParts[0]) {
				...
				case '/groupinvite':
					return this.replaceCommandGroupinvite(textParts);
				case '/invite':
					return this.replaceCommandInvite(textParts);
				case '/invitepv':
					return this.replaceCommandPvInvite(textParts);
				case '/inviteto':
				...
				case '/chnlchange':
					return this.replaceCommandChnlchange(textParts);
				case '/banshow':
					return this.replaceCommandBanshow(textParts);
				case '/startgroup':
					return this.replaceCommandStartgroup(textParts);
				case '/joingroup':
					return this.replaceCommandJoingroup(textParts);
				case '/groupstarted':
					return this.replaceCommandGroupstarted(textParts);
	...
	replaceCommandPvInvite: function(textParts) {
	
			 // var answer = confirm (textParts[1]+" invite you in private")
		 // if (answer)
			// this.sendMessageWrapper('/join Private')
		// else 
		// return;
	},
	replaceCommandInvite: function(textParts) {

		var messageNodes = this.zapmsg;
		var textNode,messageText;
	
		textNode = messageNodes[messageNodes.length-1].getElementsByTagName('text')[0];
		messageText = textNode.firstChild ? textNode.firstChild.nodeValue : '';
				 			// var inviteText = this.lang['invite']
								...
								// .replace(
									...
									// + this.scriptLinkEncode('Private')
									// + '\');" title="'
									// + this.lang['joinChannel'].replace(/%s/, 'Private')
									// + '">'
									// + 'Private'
									// + '</a>'
								// );
			...
			// + inviteText
			// + '</span>';
		if (Date.parse(Date()) - Date.parse(messageNodes[messageNodes.length-1].getAttribute('dateTime')) < 5000 && messageText.substring(0,7) == '/invite')//&& messageText.substring(0,13) != '/channelEnter'
		{
		 var answer = confirm (textParts[1]+" invite you in private")
		 if (answer){
		 	this.sendMessageWrapper('/join Private');
			this.sendMessageWrapper('/agree '+textParts[1]);
			return	'<span class="chatBotMessage">'
					+ this.userName + ' change channel '
					+ '</span>';
			}
		 else {
			this.sendMessageWrapper('/abort '+textParts[1]);
			return	'<span class="chatBotMessage">'
					+ 'denied'
					+ '</span>';
			  }		
	...
	replaceCommandGroupinvite: function(textParts) {
		var messageNodes = this.zapmsg;
		var textNode,messageText;
		...
		messageText = textNode.firstChild ? textNode.firstChild.nodeValue : '';
			...
			// + inviteText
			// + '</span>';
		if (Date.parse(Date()) - Date.parse(messageNodes[messageNodes.length-1].getAttribute('dateTime')) < 5000 && messageText.substring(0,12) == '/groupinvite')//&& messageText.substring(0,13) != '/channelEnter'
		{
		 var answer = confirm (textParts[1]+", " +textParts[2]+" and " +textParts[3] + " invite you in Group Show")
		 if (answer){
		 	this.sendMessageWrapper('/join Private');
			this.sendMessageWrapper('/agree '+textParts[1]+" " +textParts[2]+" " +textParts[3]);
	...
	replaceCommandChnlchange: function(textParts) {
					 			// var inviteText = this.lang['invite']
								// .replace(/%s/, textParts[1])
								// .replace(
									// /%s/,
		...
		textNode = messageNodes[messageNodes.length-1].getElementsByTagName('text')[0];
		messageText = textNode.firstChild ? textNode.firstChild.nodeValue : '';
		if (Date.parse(Date()) - Date.parse(messageNodes[messageNodes.length-1].getAttribute('dateTime')) < 5000 && messageText.substring(0,13) != '/channelEnter'){//&& messageText.substring(0,13) != '/channelEnter'
		this.sendMessageWrapper('/join Private');
		this.gogotime(textParts[1],1,this.ajaxURL,this.lastID);
		}
	},
	...
	replaceCommandStartgroup: function(textParts) {
	...
	replaceCommandJoingroup: function(textParts) {
	...
	replaceCommandGroupstarted: function(textParts) {
			var messageNodes = this.zapmsg;
		var textNode,messageText;
	
		textNode = messageNodes[messageNodes.length-1].getElementsByTagName('text')[0];
		messageText = textNode.firstChild ? textNode.firstChild.nodeValue : '';
		if (Date.parse(Date()) - Date.parse(messageNodes[messageNodes.length-1].getAttribute('dateTime')) < 5000 && messageText.substring(0,13) == '/groupstarted'){//&& messageText.substring(0,13) != '/channelEnter'
			document.getElementById("startgroupbutton").style.display = 'none';
	...
	replaceCommandUninvite: function(textParts) {
		...
		if (Date.parse(Date()) - Date.parse(messageNodes[messageNodes.length-1].getAttribute('dateTime')) < 5000 && messageText.substring(0,13) != '/channelEnter'){//&& messageText.substring(0,13) != '/channelEnter'
		this.sendMessageWrapper('/join Public');
		}
	...
	replaceCommandUninviteTo: function(textParts) {
		...
		if (Date.parse(Date()) - Date.parse(messageNodes[messageNodes.length-1].getAttribute('dateTime')) < 5000 && messageText.substring(0,13) != '/channelEnter'){//&& messageText.substring(0,13) != '/channelEnter'
		this.sendMessageWrapper('/join Public');
		this.gogotime(textParts[2],0,this.ajaxURL,this.lastID);
		}
	...
	gogotime: function (starttimer,ticket,myUrl,myId) {
		var timer_counter = 0;
		function gogogo()
		{
			timer_counter++;
			if (timer_counter % 3 == 0)
			{
					...
					var requestUrl = ajaxChat.ajaxURL
						+ '&lastID='
						+ ajaxChat.lastID
						+ '&pay='+timer_counter;
					if (window.XMLHttpRequest)
					...
					xmlhttp.onreadystatechange=function()
					  ...
					  if (xmlhttp.readyState==4 && xmlhttp.status==200)
						{
							if (parseFloat(xmlhttp.responseText) != '-1')
							{
			...
			var temp = Math.floor(timer_counter / (60 * 60));
			var gogohours = temp;
			temp = Math.floor((timer_counter - temp * 3600) / 60);
			var gogominutes =	((temp < 10) ? '0' : '') + temp;
			temp = timer_counter % 60;
			var gogosec =  ((temp < 10) ? '0' : '') + temp; 
			document.getElementById("currenttime").src ='image.php?t='+gogohours + ':' + gogominutes + ':' + gogosec;
			}
		
		if (ticket == 1)
			{
			timerPrv = setInterval(gogogo, 1000);
			}
		else
			{
			clearInterval(timerPrv);
			}
},

	sendtip: function(tipvalue,tipcomment){
	var requestUrl = this.ajaxURL
						+ '&lastID='
						+ this.lastID
						+ '&tip='+tipvalue+'&tipcomment='+tipcomment;
	if (window.XMLHttpRequest)
	  {// code for IE7+, Firefox, Chrome, Opera, Safari
	  xmlhttp=new XMLHttpRequest();
	  }
	  ...
	  if (xmlhttp.readyState==4 && xmlhttp.status==200)
		{
			...
			{
				ajaxChat.addChatBotMessageToChatList('don\'t enough money');
			}
		}
	  }
	xmlhttp.open("GET",requestUrl,true);
	...
	replaceCommandBanshow: function(textParts) {
	...
	if (ajaxChat.show)
		...
		if (this.userName == 'admin')
		 {
			document.getElementById("box").innerHTML = "<object width='99%' height='100%'><param name='movie' value='H264_Encoder.swf'/><param name='quality' value='high'/><param name='scale' value='maintainaspectratio'/><param name='allowFullScreen' value='true'/><embed src='H264_Encoder.swf' width='99%' height='100%' scale='maintainaspectratio' allowFullScreen='true'  quality='high' pluginspage='http://www.macromedia.com/shockwave/download/index.cgi?P1_Prod_Version=ShockwaveFlash' type='application/x-shockwave-flash'></embed></object><img id='grid_img' src=\"grip.png\" width='1%' height='100%' alt='>' onmousedown='hook(event)'></img>";
		 } else
		 {
	...
	{
		this.vojarUser = true;
		this.gogotime(0,1,0,0);
		document.getElementById("box").innerHTML = "<object width='99%' height='100%'><param name='movie' value='h265_encoder.swf'/><param name='quality' value='high'/><param name='scale' value='maintainaspectratio'/><param name='allowFullScreen' value='true'/><embed src='h265_encoder.swf' width='99%' height='100%' scale='maintainaspectratio' allowFullScreen='true'  quality='high' pluginspage='http://www.macromedia.com/shockwave/download/index.cgi?P1_Prod_Version=ShockwaveFlash' type='application/x-shockwave-flash'></embed></object><img id='grid_img' src=\"grip.png\" width='1%' height='100%' alt='>' onmousedown='hook(event)'></img>";
	},
	stopvojar: function ()
	{
		this.vojarUser = false;
		this.gogotime(0,0,0,0);
		greenshirma();
	},
	privatemessageUpdate: function ()
	{
		var requestUrl = this.ajaxURL
							+ '&lastID='
							+ this.lastID
		...
		xmlhttp.onreadystatechange=function()
		  ...
		  if (xmlhttp.readyState==4 && xmlhttp.status==200)
			{
				if (ajaxChat.pv_msg_count != parseFloat(xmlhttp.responseText))
				{
					// if(parseFloat(xmlhttp.responseText))
					// {
						// alert('ku ku epta')
					...
					// }
					if ((parseFloat(xmlhttp.responseText)>0) && (ajaxChat.userRole > 0))
					{
	...
	z_alert: function(username){
		...
		// {
			var requestUrl = this.ajaxURL
								+ '&lastID='
								+ this.lastID
								+ '&get_private_msg='+username
								+ '&get_private_msg_u2='+ajaxChat.userName;
			if (window.XMLHttpRequest)
			  {// code for IE7+, Firefox, Chrome, Opera, Safari
			  xmlhttp=new XMLHttpRequest();
			  }
			...
			xmlhttp.onreadystatechange=function()
			  ...
			  if (xmlhttp.readyState==4 && xmlhttp.status==200)
				{
					document.getElementById("pv_messages_element").innerHTML += xmlhttp.responseText.substr(0,xmlhttp.responseText.indexOf('<!--error-->'));
					// document.getElementById("WIND").style.display = 'block';
					document.getElementById("pv_messages").scrollTop = document.getElementById("pv_messages").scrollHeight
				}
			  }
			xmlhttp.open("GET",requestUrl,true);
			xmlhttp.send();
	...
	pv_msg_KeyPress: function(event) {
		if(event.keyCode == 13 && !event.shiftKey) {
		//alert(event)
			// alert(document.getElementById("pv_nick0").innerHTML)
			this.sendMessage('/msg ' + document.getElementById("pv_nick0").innerHTML + ' ' + document.getElementById("pv_inputField").value);
			document.getElementById("pv_inputField").value = '';
			document.getElementById("pv_inputField").focus();
			try {
				event.preventDefault();
			} catch(e) {
				event.returnValue = false; // IE
	...
	,
	make_db_request: function(requestUrl){
		var array;
		if (window.XMLHttpRequest)
		  {// code for IE7+, Firefox, Chrome, Opera, Safari
		  xmlhttp=new XMLHttpRequest();
	...
	clean_unread_message: function (elems){
	var parameterString = '';
	...
	startgroup: function () {
	//alert (this.userName)
		ajaxChat.sendMessageWrapper('/startgroup '+this.userName);
	},
	joingroup: function () {
	...
	videofunction: function (param) {
		switch(param) {
			case 'refresh':
				...
				break;
			case 'onoff':
				if (document.getElementById("box").innerHTML.substr(0,4) == "<img"){
					if (this.userName == 'admin')
					 {
						document.getElementById("box").innerHTML = "<object width='99%' height='100%'><param name='movie' value='H264_Encoder.swf'/><param name='quality' value='high'/><param name='scale' value='maintainaspectratio'/><param name='allowFullScreen' value='true'/><embed src='H264_Encoder.swf' width='99%' height='100%' scale='maintainaspectratio' allowFullScreen='true'  quality='high' pluginspage='http://www.macromedia.com/shockwave/download/index.cgi?P1_Prod_Version=ShockwaveFlash' type='application/x-shockwave-flash'></embed></object><img id='grid_img' src=\"grip.png\" width='1%' height='100%' alt='>' onmousedown='hook(event)'></img>";
					 } else
			...
			case 'goprivate':
				if (this.channelName == 'Public'){
					ajaxChat.sendMessageWrapper('/invite admin');
				} else {
					ajaxChat.sendMessageWrapper('/uninvite admin');
				}
				break;

	...
	function replaceTemplateTags($tag, $tagContent) {
		switch($tag) {
			...
			case 'logoutinfo':
				...
				return $this->ajaxChat->logoutinfo();
				
			case 'SHOW_CURRENT_COINS':
			
				return $this->ajaxChat->getCurrentCoins();				
			case 'MODEL_PANEL_START_AND_BODY':
			
				return $this->ajaxChat->getModelPanel();				
			case 'MODEL_PANEL_END':
			
				return $this->ajaxChat->getModelPanelEnd();				
			case 'UNDER_VIDEO_PANEL':
			
				return $this->ajaxChat->getUVPanel();
				
			case 'USER_NAME_FROM_DB':
				return $this->ajaxChat->getJoomUsername();
